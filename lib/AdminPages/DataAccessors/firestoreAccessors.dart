...
class FirestoreAccessors{
  ...
  Future<DocumentReference> createOrder() async{
    ...
    return Auth().getFireBaseCurrentuser().then((FirebaseUser user){
      return _fStoreInstance.collection("Orders").add({'orderedBy': user.email, 'orderStatus': statusType.order_Placed.index , 'orderPlaced': timeNow});
    });
  }

  Future<QuerySnapshot> getAllOrders() async{
    return Auth().getFireBaseCurrentuser().then((FirebaseUser user){
      return _fStoreInstance.collection("Orders").getDocuments().then((QuerySnapshot orders){
        orders.documents.retainWhere((searchItems) => searchItems.data['orderedBy'] == user.email);
        return orders;
      });
    });
  }
  ...
  Future<UpdateResult> updateOrderStatus(String orderID, int statusChange){
    String timeNow = CommonWidgets.timeStampToString(Timestamp.now());
    return _fStoreInstance.collection("Orders").document(orderID).setData({'orderStatus': statusChange, 'lastUpdated': timeNow}, merge: true).then((x){
      return new UpdateResult(pass: true, remarks: "Status Updated Successfully!");
    }).catchError((error){
  ...
  Future<List<int>> getOverViewNumbers() async{
    ...
    return getAllOrders().then((QuerySnapshot orders){
      ...
      orders.documents.retainWhere((items)=> items.data['orderStatus'] == statusType.order_Complete.index);
      complete = orders.documents.length;
      incomplete = total - complete;
      List<int> overviewNumbers = [
